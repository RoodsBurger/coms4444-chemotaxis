...
public class Controller extends chemotaxis.sim.Controller {
    ...
    public ChemicalPlacement applyChemicals(Integer currentTurn, Integer chemicalsRemaining, Point currentLocation,
        ...
        // if(!isPathsEquivalent(this.myTurnPath,
        // getTurns(getOptimalPath(currentLocation, grid,
        ...
        // If not enough chemicals to direct to the end, we assume that the agent is
        // randomly roaming.
        // This checks if current cell can get to the end with the number of chemicals
        // currently.
        // If so, then we start directing it to the end.
        if (!this.myTurnPath.isEmpty() && this.totalChemicals < this.myTurnPath.size() && !this.onPathToTarget
                && this.myAllKPaths.keySet().contains(currentLocation)) {
    ...
    private Boolean closeToTurn(Point cur, List<Point> turnPath) {
        ...
        } else if (this.turnSignals.get(0) == ChemicalType.GREEN) {
            for (Point point : turnPath) {
                if (point.x == cur.x && Math.abs(point.y - cur.y) <= 15)
                    return true;
                else if (point.y == cur.y && Math.abs(point.x - cur.x) <= 15)
                    return true;
            }
        }
        return false;
    ...
    private List<Point> getOptimalPath(Point s, ChemicalCell[][] grid, Map<Point, List<Point>> allKTargetPaths,
        ...
        if (!useTurns)
            finalPath = getOptimalDirectedPath(s, grid, allKTargetPaths, nChemicals, useTurns);
